...
#include "GenHitCluster_t.h"
#include "TA2GenericCluster.h"

//---------------------------------------------------------------------------
GenHitCluster_t::GenHitCluster_t( Char_t* line, UInt_t index, Int_t sizefactor,
...
//---------------------------------------------------------------------------
GenHitCluster_t::~GenHitCluster_t( )
{
  // delete arrays
...
//---------------------------------------------------------------------------
void GenHitCluster_t::ClusterDetermine( TA2GenericCluster* cl )
{
  ...
  *fMeanPosition = *(pos[fIndex]) * wgtE;    // position = "centre"
  ...
  for( i=0,k=1; i<nhits; i++ ){
    ...
    if( IsNeighbour(j) ){                     // a neighbour of the center?
      ...
      // Optional energy weighting function
...
//-----------------------------------------------------------------------------
void GenHitCluster_t::MoreNeighbours( TA2GenericCluster* cl ){
