...
void Asymmetry( UInt_t chan_lo = 300, UInt_t chan_hi = 300,
		TString subt = "subt", Bool_t draw = kTRUE)
{
	UInt_t i;
	UInt_t col;
	Double_t eg, deg;
...
// for his bins.
//
void PlotCompSergey( UInt_t start = 0, Bool_t save == kFALSE)
{
	UInt_t i, j, bin;
	Double_t eg, deg;
	Double_t theta[20], dtheta[20], as[20], das[20];
	Double_t junk;
	Double_t x[2], y[2];
	TString name;

	gStyle->SetOptStat( 0);
	c1 = new TCanvas( "c1", "AsymCompSergey", 400, 10, 1000, 500);
	c1->Divide( 3, 2);
	...
	for ( UInt_t bin = start; bin < start + 6; bin++) {
		...
		// Get my results
		Asymmetry( spbin[bin].ch_lo, spbin[bin].ch_hi, "subt", kFALSE);
		for ( i = 0; i < 9; i++) {
			theta[i] = Asym[i].th;
			dtheta[i] = Asym[i].dth;
			as[i] = Asym[i].as;
