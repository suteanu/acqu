...
inline void TA2CalArray::ReadDecoded( )
{
  ...
  Double_t total = 0.0;                                // total energy sum
  ...
  for( i=0,k=0; i<fNhits; i++ ){                       // loop over hits
    ...
    E+=fRandom->Gaus(0.0, 0.027 * TMath::Power(E, 0.75));
    E*=1000.;
    // end smear energy
    ...
    if(fIsTime){
      T=*time++;
      T+=fRandom->Gaus(0.0, 1.03);
      TthrLow = fElement[j]->GetTimeLowThr();          // low thresh
      TthrHigh = fElement[j]->GetTimeHighThr();        // high thresh
      if( (T < TthrLow) || (T > TthrHigh) ) continue;  // time inside thresh?
      fTime[j] = T;                                    // save time
      fTimeOR[k] = T;
    }
    fHits[k] = j;                                      // store hit
    total += E;                                        // energy total sum
    k++;                                               // update # good hits
  }
