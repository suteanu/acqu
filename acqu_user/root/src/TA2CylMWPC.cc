...
void TA2CylMWPC::PostInit( )
{
  ...
  if(fClassTypeCB==1)
  {
    fCB = (TA2CB*)((TA2Analysis*)fParent);
    fPID = ((TA2CB*)fCB)->GetPID();
  }
...
void TA2CylMWPC::MakeTracks()
{
  ...
  for(i=0; i<fNIntersect[0]; i++)
  {
    ...
    for(j=0; j<fNIntersect[1]; j++)
    {
      ...
      z = vertex->Z();
      if((z < fMinVertZ) || (z > fMaxVertZ)) continue;     // z limits
      fPhiDiff[n] = phi0 - phi;
      *fPseudoVertex[n] = *vertex;
      fPseudoVertX[n] = vertex->X();
      fPseudoVertY[n] = vertex->Y();
      fPseudoVertZ[n] = vertex->Z();
      fPseudoVertR[n] = TMath::Sqrt(vertex->X()*vertex->X() + vertex->Y()*vertex->Y());
...
FinishedTracks:
  ...
  n = 0;
  for(i=0; i<fNTrack-1; i++)
    for(j=i+1; j<fNTrack; j++)
    {
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if(!strcmp("ClassTypeCB", sWord))
  {
    sscanf(line, "%*s %d", &fClassTypeCB);
    return;
  }
}
